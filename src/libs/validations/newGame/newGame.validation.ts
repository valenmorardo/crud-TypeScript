...
export const validateNewGameFields = (
	...
	if(typeof name !== 'string') {
		return res.status(400).send({
			error_message: "El nombre debe estar en formato texto.",
			status: {
				code: 400,
	...
	if(!validator.isAscii(name)) {
		return res.status(400).send({
			error_message: "El nombre contiene caracteres invalidos. Solo se permiten letras y/o numeros.",
			status: {
				code: 400,
				msg:httpStatusCodes[400]
			}
	...
	if(!validator.isAscii(description)) {
		return res.status(400).send({
			error_message: "La descripcion contiene caracteres invalidos. Solo se permiten letras y/o numeros.",
			status: {
				code: 400,
				msg:httpStatusCodes[400]
			}
	...
	if(!genres.every(genre => Object.values(genres_videogames).includes(genre as genres_videogames))) {
		return res.status(400).send({
